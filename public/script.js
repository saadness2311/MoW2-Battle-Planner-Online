...
// --- Realtime bridge: send/receive actions via room WebSocket (if present) ---
(function(){
  // When main app performs an action, it can dispatch a custom event 'local_action' with detail = { actionType, data }
  // Bridge will forward it to server if connected. And incoming 'remote_action' events will be emitted as 'apply_remote_action' for main app.
  function forwardToServer(action){
    if(window._roomWs && window._roomWs.send){
      window._roomWs.send({ type:'action', payload: action });
    }
  }
  document.addEventListener('local_action', e=>{
    try{ forwardToServer(e.detail); }catch(err){ console.warn('forward action err', err); }
  });
  document.addEventListener('remote_action', e=>{
    // Re-dispatch so main script can apply it
    document.dispatchEvent(new CustomEvent('apply_remote_action', { detail: e.detail }));
  });

  // When room joined notify main app (if it listens)
  document.addEventListener('room_joined', e=>{
    document.dispatchEvent(new CustomEvent('room_ready', { detail: e.detail }));
  });
  // expose
  window._realtimeBridge = { forwardToServer };
})();
