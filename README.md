...
- Авторизация только через Supabase Auth (никнейм ≥ 3 символов, пароль ≥ 1). Никнеймы уникальны и хранятся в таблице `profiles`.
...
- `room_units`: все юнитные иконки/символы с координатами, командой и слотом.
- `room_drawings`: линии фронта и зоны противника (polyline/polygon).
- `room_symbols`: запасной слой простых маркеров.
- `room_state`: агрегированное состояние карты по эшелонам (для бэкапов).
- `chat_messages`: сообщения комнаты (лимит 1/сек на клиенте).
- `action_logs`: технические логи.
- Публикация `supabase_realtime` подключена к основным таблицам, RLS включает чтение для `authenticated` и изменения только для администратора/редактора/ходящего игрока.

Полный DDL и политики — в [`supabase_schema.sql`](./supabase_schema.sql).

## Как работать в приложении

1. Зарегистрируйте ник на `/register` → Supabase создаст профиль. Повторный логин через `/login`.
2. На `/rooms` создайте комнату (опциональный пароль) или войдите в существующую. Создатель автоматически становится редактором и владельцем хода.
3. В комнате:
...
- `app/` — страницы Next.js (`/login`, `/register`, `/rooms`, `/room/[id]`).
